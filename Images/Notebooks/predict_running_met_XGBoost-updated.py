...
# ### Predicting the `running` MET using the XGBoost Regression Model
...
# ### Get the running activities from the entire dataframe
...
def find_optimal_features(features):
    ...
    rfe = RFE(estimator=xgbr, n_features_to_select=6, step=1)
...
# columns_by_rfe = ['sum_mag_acc', 'weight_kg', 'length_cm', 'is_sporter', 'age_category', 'mean_speed']
# X = all_df[['sum_mag_acc', 'weight_kg', 'length_cm', 'meets_balance_guidelines', 'mean_speed']]
# columns_by_rfe = ['sum_mag_acc', 'weight_kg', 'length_cm', 'is_sporter', 'age_category', 
