...
# ### Predicting the `cycling` MET using the XGBoost Regression Model
...
cycling_df = all_df.loc[all_df['activity'] == 'fietsen zwaar']
# cycling_heavy_df = all_df.loc[all_df['activity'] == 'fietsen zwaar']
# cycling_df_combined = pd.concat([cycling_light_df, cycling_heavy_df])
cycling_df
...
def create_respondent_dataframe(respondents):
    ...
    for respondent in respondents:
        resp_df = cycling_df[(cycling_df['respondent_code'] == respondent)]
...
# ### Test Respondents
# Decided to remove all `mean_met` values under 1 since this is the average MET value for sitting. 
...
all_df_test = all_df_test.loc[all_df_test['mean_met'] >= 1]
...
# all_features = ['sum_mag_acc', 'weight_kg', 'length_cm', 
#                 'is_sporter', 'age_category', 'meets_balance_guidelines', 
#                 'meets_activity_guidelines', 'gender', 'mean_speed']
...
def find_optimal_features(features):
    ...
    rfe = RFE(estimator=xgbr, n_features_to_select=8, step=1)
...
# X = all_df[['sum_mag_acc', 'weight_kg', 'age_category', 'meets_balance_guidelines', 'estimated_level', 'mean_speed']]
# X = all_df[['sum_mag_acc', 'weight_kg', 'is_sporter', 'meets_balance_guidelines', 'mean_speed']]
...
final_xgb_model = apply_randomized_search_hyperparameter_tuning(random_grid)
final_xgb_model
...
dump(final_xgb_model, 'cycling_heavy.dat')
